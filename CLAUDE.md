...
- `frontend/` holds the React app.
  ...
  - **Guided setup** (`#/setup`, `SetupPage.jsx`): a step rail Menu → Recipes → Your prices (optional) → Sales → Results. Each step reuses an existing screen: the menu/invoice/sales review via `ImportReview.jsx`, and `RecipeStep.jsx`, which shows `RecipeEditor` for one dish at a time from `needs_recipe` (Save moves on; Skip for now). Once the owner uses Estimate with AI, the next dish's draft is fetched in the background and handed to `RecipeEditor` as `initialDraft` (still only a draft until Save). Progress comes from the data (setup status), never a stored wizard position, so leaving and returning just works. Start fresh opens it; until the required steps are done the Overview shows `SetupChecklist.jsx` with Continue setup (step logic and `firstUnfinished` in `src/setup.js`). Settings (`SettingsPage.jsx`, bottom of the sidebar) has Start fresh and Load demo data.
  - Uploads share `src/imports.js` (`UPLOADS`, `readUpload`), `components/UploadButton.jsx` and `components/ImportReview.jsx` between the Imports and Setup pages.
