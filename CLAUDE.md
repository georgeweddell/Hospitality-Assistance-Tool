...
- `backend/` holds the FastAPI app. The files are flat, with no sub-packages.
  ...
  - Each action also says **what to change** (`proposed_change`, agreed with George 25 Sep 2026; the impact formulas are unchanged): Plowhorse and Dog get today's price, the margin gap per plate at today's price and the target price that closes it (= plate cost + the category's average margin, e.g. Margherita £11.50 → £12.40, or cut £0.90 from the plate); Puzzle gets the extra plates over the period to reach the popularity line (rounded up) and per day. Exact targets, not rounded to menu prices; including VAT like every price. `build_action_list(..., today=None)` lets tests fix "today". Shown on tickets, the Actions table's Change column and the dish page's price tile (wording in `actionText.proposal`), and given to the report agent as facts.
  ...
  - `price_changes.py`: ingredient price changes (`find_price_changes`: a day when `best_price` moves, for ingredients in recipes of dishes on the menu today, from the start of the period to today; old/new price, % change, £ per plate for each dish, and the effect at the period's plates sold) and menu price changes (`find_menu_price_changes`). Alerts are rises of 5% or more (`ALERT_PERCENT`); falls and small rises are listed but not alerts; no dismissing (they drop out of the window). Routes `GET /price-changes` (adds `after_period`) and `GET /menu-price-changes`. Shared with the report agent. Agreed with George 25 Sep 2026.
  - `own_prices.py`: the share of a plate cost from the restaurant's own prices (invoice, supplier list or typed in; benchmarks aren't): `own_share` per dish (in `/recipes` rows) and `menu_own_share` (one plate of each dish on the menu; `own_cost_share` in setup status). Margherita on the demo: 90%.
  - `report_tools.py`: the report agent's read-only tools (period summary, ranked actions, one dish in detail, ingredient price changes, sales by weekday / category / dish, data gaps). Each number they find is a numbered fact (`Facts`, formatted only by `format_value`); `run_tool` turns Claude's mistakes (unknown tool, bad input) into messages rather than errors.
  - `report_agent.py`: the report agent's loop (see "report agent" below). Claude (Sonnet 5) calls the tools in `report_tools.py`, at most 12, then `write_report`; `check_report` rejects any digit in its text or any cited fact id that doesn't exist, sends the problems back once, then fails. `run_report_job` runs it in a thread and saves progress to the `Report` row.
...
- `frontend/` holds the React app.
  ...
  - Reports (`ReportsPage.jsx`, `#/reports` and `#/reports/<id>`; not in the nav, reached from the Overview's Generate report button and a Reports button on Insights): see "report agent" below. `GenerateReportButton` starts one for the chosen period and opens it.
  ...
  - Price changes on screen: `src/priceChanges.js` (`usePriceChanges`, `priceMove` "£7.40 → £8.20/kg", `percentMove`) and `components/PriceChanges.jsx` (`PriceRiseList`). The Overview ends with **ingredient price rises** (the alerts, each with its £ effect, date, "after the period" when it is, and the £ per plate on the dishes using it) and **menu price changes**. The Ingredients page puts a ▲/▼ chip by a changed price (tomato for an alert) and shows the recipe-cost share in its tile; the dish page shows "N% own prices" on the plate cost tile and a chip for each price change that hit the dish; the Recipes table has an "Own prices" column.
...
## Now: report agent (agreed with George 25 Sep 2026)
