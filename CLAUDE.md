...
- `frontend/` holds the React app.
  ...
  - **Design system (Docket): `src/index.css`.** Every colour (tokens on `:root`), the fonts (Bricolage Grotesque for headings, figures and text; DM Mono for labels, buttons and table numbers) and the shared component classes live there: `btn btn-primary|secondary|danger|sm`, `input`, `field` + `label`, `card` / `card-header` / `card-body`, `table` (+ `row-link`), `chip chip-warn|muted|accent`, `count`, `tabs` / `tab`, `segmented`, `alert-error`, `alert-info`, `empty`, `progress` + `progress-bar`, `chart-tip` (chart tooltips: `--tip-edge` sets the coloured edge; tooltips are shown without the Recharts glide, `isAnimationActive={false}`), `page-title`, `section-title`, `stat-value`, `num`, `link`. **Use these; don't write one-off button, input, table or card styles in a component.** Change a look in `index.css`, once.
  ...
  - **Insights** (`InsightsPage.jsx`): one menu-engineering chart (`QuadrantChart.jsx`) with a dropdown filter that starts on **All categories**; the highlights and figures follow the filter. Because each category is judged against its own lines, the All view plots every dish **relative to its own category's lines** (across: its share as a % of the popularity line, 100% = on the line; up: margin above or below its category's average, £0 = on the line), so one pair of lines is right for every dish and each colour matches its zone. A single category shows real shares and £ margins. The chart is re-keyed per filter (Recharts otherwise asks for labels of dots that have gone). Only the highlighted dishes are named on the chart (every dish shows on hover; a click opens it), placed by `src/labelPlacement.js`. Beside it: Top earner (margin × units), Biggest opportunity (top £ action), and quadrant moves since the previous period; underneath, the category's contribution, sales, margin per dish sold and units against the previous period. The totals come from `src/figures.js` (`summarise`, shared with the Overview); the ▲/▼ marker is `components/Delta.jsx`.
...
## Now: "deli counter" redesign (agreed 25 Sep 2026, before the report agent)
...
Steps, one commit each: (1) tokens, fonts, restyled shared classes in `index.css` (done); (2) top nav in `Layout.jsx`; (3) shared pieces: `.tile` colour tiles (not `.block`: that is a Tailwind utility), `.figure`, `TicketRail`/`Ticket`, `Stamp`, lowercase big page titles; (4) pages one at a time: Overview, Actions, Menu (category blocks; Recipes tab stays a table), dish page, Ingredients, Sales, Imports + reviews, Insights (tinted zones, pill labels); (5) rewrite the design-system notes above.
